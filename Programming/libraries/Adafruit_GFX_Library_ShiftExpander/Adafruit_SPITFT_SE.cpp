...
void Adafruit_SPITFT::initSPI(uint32_t freq, uint8_t spiMode) {
  ...
  if (_cs >= 0) {
    pinMode(_cs, OUTPUT);
    ShiftExpander::digitalWriteExpanded(_cs, HIGH); // Deselect
  }
  pinMode(_dc, OUTPUT);
  ShiftExpander::digitalWriteExpanded(_dc, HIGH); // Data mode
  ...
  if (connection == TFT_HARD_SPI) {
    ...
    ShiftExpander::digitalWriteExpanded(swspi._mosi, LOW);
    pinMode(swspi._sck, OUTPUT);
    ShiftExpander::digitalWriteExpanded(swspi._sck, LOW);
    if (swspi._miso >= 0) {
    ...
    for (uint8_t i = 0; i < NUM_DIGITAL_PINS; i++) {
      if ((PORTreg_t)portOutputRegister(digitalPinToPort(i)) ==
          tft8.writePort) {
        pinMode(i, OUTPUT);
        ShiftExpander::digitalWriteExpanded(i, LOW);
      }
    }
    ...
    ShiftExpander::digitalWriteExpanded(tft8._wr, HIGH);
    if (tft8._rd >= 0) {
      pinMode(tft8._rd, OUTPUT);
      ShiftExpander::digitalWriteExpanded(tft8._rd, HIGH);
    }
  }
  ...
  if (_rst >= 0) {
    ...
    pinMode(_rst, OUTPUT);
    ShiftExpander::digitalWriteExpanded(_rst, HIGH);
    delay(100);
    ShiftExpander::digitalWriteExpanded(_rst, LOW);
    delay(100);
    ShiftExpander::digitalWriteExpanded(_rst, HIGH);
    delay(200);
  }
...
#endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
  ShiftExpander::digitalWriteExpanded(swspi._mosi, HIGH);
#if defined(ESP32)
...
#endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
  ShiftExpander::digitalWriteExpanded(swspi._sck, HIGH);
#if defined(ESP32)
  for (volatile uint8_t i = 0; i < 1; i++)
    ;
#endif // end ESP32
...
#endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
  ShiftExpander::digitalWriteExpanded(tft8._wr, LOW);
  ShiftExpander::digitalWriteExpanded(tft8._wr, HIGH);
#endif // end !USE_FAST_PINIO
}
...
#endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
  ShiftExpander::digitalWriteExpanded(tft8._rd, LOW);
#endif // end !USE_FAST_PINIO
}
