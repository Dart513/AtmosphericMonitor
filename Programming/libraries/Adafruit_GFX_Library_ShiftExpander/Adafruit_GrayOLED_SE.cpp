...
#include "Adafruit_GrayOLED_SE.h"
...
void Adafruit_GrayOLED::oled_command(uint8_t c) {
  if (i2c_dev) {                // I2C
    ...
    ShiftExpander::digitalWriteExpanded(dcPin, LOW);
    spi_dev->write(&c, 1);
  }
}
...
bool Adafruit_GrayOLED::_init(uint8_t addr, bool reset) {
  ...
  if (reset && (rstPin >= 0)) {
    ...
    ShiftExpander::digitalWriteExpanded(rstPin, HIGH);
    delay(10);                  // VDD goes high at start, pause
    ShiftExpander::digitalWriteExpanded(rstPin, LOW);  // Bring reset low
    delay(10);                  // Wait 10 ms
    ShiftExpander::digitalWriteExpanded(rstPin, HIGH); // Bring out of reset
    delay(10);
  }
